...
- **Real auth.** ✅ **DONE 2026-05-06** — full JWT migration shipped (see JWT_MIGRATION_PLAN.md). Family portal now uses HttpOnly `tb_session` cookies set by Next route handlers on the Vercel origin; the JWT never enters client JS; Edge middleware gates `/dashboard` and `/seniors/*`. **2026-08-09 update:** seniors on mobile now also have real email+password accounts (`ACCOUNTS_AND_PREMIUM_PLAN.md`) — this was the prerequisite for Premium billing. The family portal itself is unchanged and still uses invite codes, not email/password; that's still future work if we ever want a family member to sign in from a second device without a fresh code.
