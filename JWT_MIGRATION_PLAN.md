...
| **E** | API release N+1  | Pre-handler stops accepting `X-User-Id`. CORS `allowedHeaders` drops `X-User-Id`. The mobile/web client code that still references the legacy header (now in fallback paths) gets removed in mobile release N+2 / web release N+1. | Re-deploy API release N. The fallback paths in mobile/web haven't been touched yet at this point so reverting the API is sufficient. |
| **E.1** | API hotfix     | Original Stage E plan also turned `/v1/auth/exchange` into a 410 Gone tombstone. We hit a real recovery scenario in production (an iOS phone whose Keychain JWT silently failed to persist — see TECH_DEBT.md), so we restored exchange as the recovery escape hatch. The pre-handler stays Bearer-only; only the mint helper is back. Rate-limited per IP (5/min, 30/hour) so it can't be abused for userId enumeration. | n/a (additive). |
...
- **`apps/api/src/routes/auth.ts:/v1/auth/exchange`** is alive again as a recovery escape hatch (Stage E.1). When client-side state gets corrupted in a way that costs the JWT but not the userId — e.g. iOS Keychain misbehavior — exchange is the only way back without forcing a reinstall. Rate-limited to neutralize abuse. Long-term we may want to lock this behind a per-user secondary factor (device-attested key, magic link, etc.) before any non-beta launch.
